...
def open_spotify():
    subprocess.run(["C:\Users\Hrishi\AppData\Local\Microsoft\WindowsApps\SpotifyAB.SpotifyMusic_zpdnekdrzrea0\Spotify.exe"])

def open_chrome():
    subprocess.run(["AutoIt3.exe", "path_to_your_script\\launch_chrome.au3"])

def run_assistant():
    command = take_command()
    if command is None:
        print("No command recognized, try again.")
        return  # Return early if no valid command is detected

    # Example voice commands for opening applications
